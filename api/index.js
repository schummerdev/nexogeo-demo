...
module.exports = async function handler(req, res) {
  ...
  if (!route && req.url) {
    const pathMatch = req.url.match(/^\/api\/([^?\/]+)/); // [^?\/]+ não captura após "/"
  ...
  if (route === 'participantes') {
    try {
      ...
      if (req.method === 'GET') {
        ...
        if (id) {
          // Buscar participante específico por ID
          const result = await query(`
      ...
      if (req.method === 'POST') {
        ...
        if (!promocao_id || !nome || !telefone) {
          return res.status(400).json({
            success: false,
            message: 'Promoção ID, nome e telefone são obrigatórios',
            timestamp: new Date().toISOString()
          });
        }
      ...
      if (req.method === 'PUT') {
        ...
        // Tratar campos numéricos vazios - corrigir parsing para evitar NaN
        const latitudeParsed = (latitude === '' || latitude === null || latitude === undefined) ? null :
          isNaN(parseFloat(latitude)) ? null : parseFloat(latitude);
        const longitudeParsed = (longitude === '' || longitude === null || longitude === undefined) ? null :
          isNaN(parseFloat(longitude)) ? null : parseFloat(longitude);
        ...
        // Log de auditoria removido - será feito pelo frontend via auditService
        console.log('📝 Participante atualizado - auditoria será registrada pelo frontend');
